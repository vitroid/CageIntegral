...
def Avalue(Temp, mass):
    return -pc.NkB*Temp*(3./2.)*np.log(mass*0.001 / pc.NA * 2.0 * np.pi * pc.kB*Temp / pc.h**2)
...
def Cvalue(T,ixx,iyy,izz):
    ...
    return -pc.NkB*Temp*((1./2.)*np.log(ixx*iyy*izz) + (3./2.)*np.log(2.0 * np.pi * pc.kB*Temp / pc.h**2))
...
def test():
    ...
    for T in range(273,293):
        logger.debug (T,chempot(T,101326)+Avalue(T,16),chempot(T,antoine.VaporPressure(T, A,B,C)))
