...
def Avalue(Temp, mass):
    return -pc.NkB * Temp * \
        (3. / 2.) * np.log(mass * 0.001 / pc.NA * 2.0 * np.pi * pc.kB * Temp / pc.h**2)
...
def Cvalue(Temp, ixx, iyy, izz):
    ixx *= 0.001 / pc.NA * 1e-10**2
    iyy *= 0.001 / pc.NA * 1e-10**2
    izz *= 0.001 / pc.NA * 1e-10**2
    return -pc.NkB * Temp * ((1. / 2.) * np.log(ixx * iyy * izz) +
                             (3. / 2.) * np.log(2.0 * np.pi * pc.kB * Temp / pc.h**2))


def Cvalue2(Temp, ixx):
    ixx *= 0.001 / pc.NA * 1e-10**2
    return -pc.NkB * Temp * \
        (np.log(ixx) + np.log(2.0 * np.pi * pc.kB * Temp / pc.h**2))
...
def StericFix(Temp, mass, symm, moi):
    ...
    if ixx == 0:
        ...
        # return value + IntegrationFix(Temp,dimen) + SymmetryFix(Temp, symm) +
        # Cvalue2(Temp,ixx)
